...
locals {
  ports_to_open = [
    ...
    "3260", "5900-${tostring("5900" + var.hv_node_count)}", "6080", "6443", "6444", "8181", "8443", "8444", "8472",
...
resource "azurerm_network_security_rule" "allow_inbound" {
  for_each = toset(local.ports_to_open)

  name = "${var.prefix}-allow-inbound-${each.key}"
  priority = 100 + index(local.ports_to_open, each.key)
  direction                   = "Inbound"
  access                      = "Allow"
  protocol                    = "*"
  source_port_range           = "*"
